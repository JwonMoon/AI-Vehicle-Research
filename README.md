...
| 주제 | 요약 | 작성 | 핵심 문서 |
|---|---|---|---|
| [vehicle-data-flywheel](research/vehicle-data-flywheel/) | 자율주행 데이터 플라이휠 해부 — 롱테일·분포 변화·성능 회귀·실도로 검증 한계에서 출발해 온보드 선별→업로드→검색·자동 라벨링→학습·합성 데이터(재생·재구성·생성)→시뮬·실차 검증→OTA의 9 마디 구조, NVIDIA 일반 AI 플라이휠과의 6가지 차이, 2021 데이터 엔진 vs 2026 플라이휠 부품 비교, 가상 실패 사례로 따라가는 4단계 기술(openpilot 코드 확인), Tesla·Waymo·Mobileye·Wayve·Momenta·현대차/42dot·NVIDIA·Applied Intuition·TIER IV 유형별 비교와 2025~26 동향, 병목 4(발견·검증·비용·규제)→경쟁력 5 연결, 그림 10장 | 2026-09 | [보고서](research/vehicle-data-flywheel/vehicle-data-flywheel.md) · [웹 버전](research/vehicle-data-flywheel/vehicle-data-flywheel.html) · [출처 기록](research/vehicle-data-flywheel/reference/references.md) · [폴더 안내](research/vehicle-data-flywheel/README.md) |
