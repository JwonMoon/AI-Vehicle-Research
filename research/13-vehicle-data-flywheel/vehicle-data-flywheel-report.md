# 자율주행 데이터 플라이휠 — 조사 보고서(개조식)

### 실도로 데이터가 모델을 키우고, 그 모델이 다시 더 좋은 데이터를 만드는 순환 구조의 필요성·구조·기술·기업 전략·병목을 항목별로 정리

| | |
|---|---|
| **작성일** | 2026-09-23 (조사 기준일 2026-09-22) |
| **문서 성격** | 기술 조사 보고서, 개조식. 같은 내용의 줄글 판은 [vehicle-data-flywheel.md](vehicle-data-flywheel.md)이며, 두 판의 사실·수치·출처·그림은 동일함 |
| **다루는 것** | 1장 데이터 플라이휠이 필요한 이유 · 2장 차량·클라우드·검증 환경을 잇는 전체 구조와 일반 AI 플라이휠과의 차이 · 3장 실도로 데이터가 모델 개선으로 이어지는 단계별 기술(가상 실패 사례 1건으로 추적) · 4장 기업별 전략과 2025~2026 동향 · 5장 병목·사업 경쟁력·진화 방향 |
...
4. **차량 플라이휠은 일반 AI 플라이휠과 여섯 가지가 다름.** ① 데이터가 차 안에서 태어나 업로드 예산에 묶임 ② 정답이 개입·사고 같은 물리적 결과임 ③ 배포 앞에 안전 인증 관문이 있음 ④ 업데이트가 규제(UN R156)에 묶임 ⑤ 시뮬레이터가 필수임 ⑥ 개인정보·데이터 국외 반출 규제를 받음.
5. **2021년 "데이터 엔진"과 2026년 플라이휠은 같은 원, 다른 부품임.** 사람이 짠 트리거 → 임베딩 검색, 사람 라벨링 → 파운데이션 모델 자동 라벨링, 로그 재생 → 3D 재구성·생성형 월드모델, 모방학습 → 폐루프 강화학습, 연 단위 릴리스 → 주 단위 OTA로 이동 중임.
...
- 플라이휠의 경쟁력은 "한 바퀴를 얼마나 자주·싸게·믿을 수 있게 돌리느냐"로 귀결됨. 병목(데이터 발견·검증·비용·규제)이 이 세 변수를 막음.
- 병목의 위치가 "사람 손"(트리거 작성·라벨링·시나리오 작성)에서 "GPU와 데이터 파이프라인"으로 이동함. 자동 라벨링·재구성·생성이 그 원인임.
- 플라이휠의 첫 마디(온보드 선별·섀도 실행)와 마지막 마디(OTA·규제 기록)는 차 안에 있으므로, 차량 HPC 플랫폼 설계 조건이 됨(5.7절).

---
## 1. 배경

> **개요.** 자율주행에서 데이터 플라이휠이 주목받는 이유를 설명함. 데이터 플라이휠의 개념(1.1), 규칙 기반에서 학습 기반 시스템으로의 진화(1.2), 기존 AI 학습 방식의 네 가지 한계(1.3), 지속적 closed-loop learning의 필요성(1.4) 순서임.
>
> **핵심 결론.** ① 주행 소프트웨어가 사람이 쓴 규칙에서 데이터로 학습한 모델로 바뀌었고, 모델은 데이터가 없는 상황에서는 무엇을 해야 할지 모름. ② 사고를 결정하는 것은 드문 상황이고, 드문 상황은 지역·날씨·차종이 바뀌면 또 달라지며, 새 모델은 예전 상황을 잊을 수 있음. ③ 실도로 주행으로 안전을 증명하기엔 필요한 거리가 너무 길므로, "차에서 데이터를 모아 → 고쳐서 → 시뮬레이션으로 확인하고 → 다시 차에 보내는" 루프를 멈추지 않고 돌려야 함.
...
- **과거 구조**: 자율주행 소프트웨어는 오랫동안 사람이 쓴 규칙의 집합이었음. "앞차와 거리가 이만큼이면 감속", "차선이 이렇게 보이면 중앙 유지" 같은 조건문이 수십만 줄 쌓임.
- **전환 사례**
...
> **시사점.** 병목이 "사람의 분석·코딩 시간"에서 "데이터 파이프라인과 GPU"로 옮겨감. 루프를 얼마나 자주·싸게 돌리느냐가 성능을 결정함.

### 1.3 기존 AI 학습 방식의 한계 (데이터 플라이휠이 주목받는 이유)

- **전제**: 학습 기반 시스템은 규칙이 아니라 데이터로만 고칠 수 있음(1.2절). 따라서 문제는 "어떤 데이터가, 왜 부족한가"로 바뀜.
- **네 가지 한계**: 드문 상황이 사고를 좌우하는 롱테일(1.3.1), 장소·날씨가 바뀌면 성능이 떨어지는 분포 변화(1.3.2), 재학습이 예전 능력을 지우는 성능 회귀(1.3.3), 실도로 데이터로는 안전을 증명할 수 없는 문제(1.3.4).
- **공통점**: 넷 다 "한 번 모은 데이터로 한 번 학습해 배포하는" 기존 방식으로는 풀리지 않음. 이것이 데이터 플라이휠이 주목받는 이유임.
...
- **정의**: 한 도시에서 배운 모델을 다른 도시에 가져가면 성능이 떨어짐. 이를 **분포 변화(distribution shift)** 라 함. 학습 데이터와 실제 데이터의 "분포"(어떤 상황이 얼마나 자주 나오는가)가 다르다는 뜻임.
- **수치로 확인된 사례**
...
> **시사점.** 실패 사례는 한 번 고치고 버리는 것이 아니라 영구 회귀 세트로 축적해야 함. 이것이 5장의 "장기 검증 자산"임.
...
|---|---|---|
| 자율주행차가 사람보다 사망률이 낮다 | 무사고 **2억 7,500만 마일** | 95% 신뢰 수준 |
| 사람보다 20% 더 안전하다 | **110억 마일** | 95% 신뢰·80% 검정력 |
...
> **시사점.** 실주행 거리는 안전 통계의 재료일 뿐 증명 수단이 아님. 시뮬레이션과 시나리오 기반 검증이 플라이휠의 필수 마디가 되는 근거임.

### 1.4 지속적 closed-loop learning의 필요성

- **1.3의 네 가지 한계를 정리하면** 한 방향을 가리킴
...
> **시사점.** 플라이휠은 선택이 아니라 학습 기반 자율주행의 전제 조건임. 다음 장은 그 부품(마디)을 하나씩 펼침.

---
## 2. 차량·클라우드·검증 환경을 연결하는 데이터 플라이휠 생태계

> **개요.** 차량 데이터 플라이휠의 부품(마디)을 펼치고, 챗봇 같은 일반 AI 플라이휠과의 차이, 그리고 2021년 방식과 2026년 방식의 차이를 비교함.
>
> **핵심 결론.** ① 일반 AI 플라이휠은 "생산 로그 → 데이터셋 → 미세조정 → 평가 → 배포"의 루프이며, NVIDIA가 이를 소프트웨어 블루프린트로 공개함. ② 차량 플라이휠은 여기에 "차 안에서의 선별·업로드", "시뮬레이션 검증", "안전 인증 관문", "OTA 규제"가 끼어들어 원이 더 길고 관문이 더 많음. ③ 2021년의 데이터 엔진과 2026년의 플라이휠은 원의 모양은 같지만 부품이 바뀜. 사람이 짠 트리거 → 임베딩 검색, 사람 라벨링 → 파운데이션 모델 자동 라벨링, 로그 재생 → 3D 재구성과 생성형 월드모델, 연 단위 릴리스 → 주 단위 OTA.
...
- **세 근거와 마디의 대응**
  - 서베이 7단계 "(I) 수집 → (II) 저장 → (III) 선별·전처리 → (IV) 라벨링 → (V) 학습 → (VI) 시뮬레이션·테스트 검증 → (VII) 실세계 배포" = ②③④⑤⑥⑧.
  - ①(온보드 선별·섀도 모드) = Tesla 데이터 엔진. ⑦(실차 검증) = Waymo 사례. ⑧의 안전 관문·UN R156 = 규제 자료.
...
- **AV 3.0 파이프라인과 그림 7의 차이**
  ...
  - 사람(Users)이 검색과 증강 도구에 붙어 있음. "사람이 고르고 만든다"는 3.2·3.4절 흐름과 같음.
...
> **시사점.** 차량 플라이휠의 고유 마디 네 개(①⑥⑦⑧)는 모두 "관문"임. 원을 빨리 돌리는 것과 관문을 통과하는 것을 동시에 설계해야 함.

### 2.3 일반 AI 플라이휠과 다른 여섯 가지

| # | 차이 | 일반 AI | 차량 | 근거 |
|---|---|---|---|---|
| 1 | **데이터가 태어나는 곳과 예산** | 서버 로그. 전부 저장 가능 | 차 안. 카메라 원본을 다 올릴 수 없어 차에서 골라야 하고 통신·저장 예산이 있음 | Tesla HW4 카메라는 시간당 약 20GB 생성(커뮤니티 추정 ⚠️); Mobileye REM은 km당 10KB로 설계 📰; comma는 마일당 $0.003 미만으로 서비스 비용을 맞춤 📰 |
...
> **시사점.** 일반 플라이휠은 "얼마나 빨리 도는가"의 문제이고, 차량 플라이휠은 "관문을 통과하면서도 계속 돌게 하는가"의 문제임.

### 2.4 기존 방식과 최근 방식 — 같은 원, 다른 부품

- **기존 방식의 교과서 = Tesla 2021 AI Day 데이터 엔진** 📰 ([CleanTechnica](https://cleantechnica.com/2021/08/27/how-teslas-autopilot-team-refines-an-unfathomable-amount-of-data-is-pretty-cool/), [Electrek](https://electrek.co/2021/12/01/tesla-releases-new-footage-auto-labeling-tool-self-driving/)).
...
|---|---|---|---|
| 데이터 발굴 | 사람이 조건을 코드로 쓴 트리거(Tesla 221개) 📰 | **임베딩 검색**: 영상을 숫자 벡터로 바꿔 놓고 "비 오는 밤 공사 구간"처럼 글로 검색. NVIDIA Cosmos Dataset Search는 약 800만 영상으로 학습한 영상-텍스트 임베딩(Cosmos-Embed1)을 씀 📰; Voxel51 FiftyOne 📰 | 무엇을 찾을지 미리 코드로 정하지 않아도 됨 |
| 라벨링 | 사람 라벨링 회사(Scale AI, 인력 24만 명) 📰; Tesla는 2022년 자체 라벨팀 감원 📰 | **파운데이션 모델 자동 라벨링**: NVIDIA Cosmos Curator가 "처리·주석·필터·중복 제거"를 맡고 🔍 ([Cosmos README](https://raw.githubusercontent.com/NVIDIA/Cosmos/main/README.md)), Cosmos Reason 같은 시각언어모델이 "왜 이렇게 운전했나"까지 글로 붙임 📰 | 라벨 비용이 사람 수가 아니라 GPU 시간에 비례 |
| 장면 재현 | 로그 재생 + HIL(dSPACE 리플레이 시스템) 📰 | **3D 재구성**: Waymo Block-NeRF는 사진 280만 장으로 샌프란시스코 한 구역(0.5km²)을 35개 신경 모델로 재구성 📰 ([arXiv 2202.05263](https://arxiv.org/pdf/2202.05263)); NVIDIA NuRec은 3D 가우시안 스플래팅으로 주행 로그를 다시 걸어 다닐 수 있는 3D 장면으로 만듦 📰 | 기록된 그대로만이 아니라 "그때 왼쪽으로 꺾었다면"을 볼 수 있음 |
| 장면 생성 | 사람이 시나리오 파일(OpenSCENARIO)을 작성 📰 | **생성형 월드모델**: Wayve GAIA-3(150억 파라미터, 10배 데이터) 📰 ([Wayve](https://wayve.ai/thinking/gaia-3/)); Waymo World Model(Genie 3 기반) 📰; NVIDIA Cosmos 📰 | 본 적 없는 상황을 영상으로 만들어 냄 |
| 학습 | 기록된 주행을 흉내 내는 모방학습(오픈루프) | **폐루프 강화학습·학습된 시뮬레이터**: NVIDIA AlpaGym은 시뮬레이터 안에서 정책이 스스로 운전하며 배움 📰; openpilot 0.11은 "학습된 시뮬레이터로 전량 학습" 🔍 | 모델이 자기 실수의 결과를 겪으며 배움 |
...
> **시사점.** 플라이휠 한 바퀴의 병목이 "사람 손"에서 "GPU와 데이터 파이프라인"으로 이동함. 3장에서 단계별 기술을, 5장에서 비용 병목을 봐야 하는 이유임.

---
## 3. 실도로 데이터를 모델 개선으로 연결하는 단계별 기술

> **개요.** 차에서 생긴 실패 하나가 어떤 단계와 기술을 거쳐 더 나은 모델이 되어 돌아오는지를, 가상의 실패 사례 하나를 끝까지 따라가며 4단계로 정리함.
>
> **핵심 결론.** ① 차 안에서는 "요약 먼저, 원본은 요청할 때"라는 두 단계 업로드와 세 종류의 트리거(규칙·모델 점수·사람)가 돎. ② 클라우드에서는 비슷한 장면을 임베딩으로 찾아 모으고, 온보드보다 큰 모델이 미래 프레임까지 보면서 정답을 소급해 붙임. ③ 부족한 장면은 재생·재구성·생성 세 갈래로 늘리고, 개루프 점수가 아니라 폐루프·사람 선호 지표로 검증한 뒤 섀도 배포와 단계적 OTA로 내보냄.
...
- **성격**: 롱테일의 전형임. 밤 + 비 + 공사 + 서 있는 사람이라는 네 조건이 겹쳤고, 각각은 흔하지만 조합은 드묾.
- **추적 방식**: 이 클립 하나가 어떻게 모델을 고치는지 네 단계로 봄.
...
- **전제: 모든 것을 올릴 수는 없음**
  - Intel 추정(2016): 차량당 하루 약 4,000GB(카메라 초당 20~60MB, 라이다 초당 10~70MB) 📰 ([Intel 2016](https://download.intel.com/newsroom/2021/archive/2016-11-15-editorials-krzanich-the-future-of-automated-driving.pdf)).
  - 최근 업계 추정: 시간당 1~5TB 📰⚠️ ([Siemens 블로그](https://blogs.sw.siemens.com/polarion/the-data-deluge-what-do-we-do-with-the-data-generated-by-avs/)).
  - 양산차 통신 요금으로는 감당할 수 없으므로 차 안에서 골라야 함.
- **오픈소스가 보여 주는 실제 설계(comma.ai openpilot)**: 양산차에 붙어 매일 도는 오픈소스 스택이라 로깅 설계를 코드로 확인 가능 🔍 (커밋 `521db4c`, 2026-09-20 기준).

| 설계 항목 | 내용 | 코드 위치 |
|---|---|---|
| 세그먼트 | 주행을 **1분 단위 세그먼트**로 자름(`SEGMENT_LENGTH = 60`) | 🔍 |
| 영상 압축 | 메인 카메라는 HEVC 5Mbps(폭 1,344픽셀 이하) 또는 10Mbps, 20fps. 따로 **저화질 카메라(qcamera)** 를 H.264 256kbps로 생성 | 🔍 `system/loggerd/loggerd.h` |
| 두 겹 로그 | 전체 로그(rlog)와 서비스별로 솎아 낸 **요약 로그(qlog)**. 차량 상태(carState)는 100Hz 중 1/10만, CAN 메시지는 100Hz 중 1/2,053만 qlog에 포함 | 🔍 `cereal/services.py` |
| 업로드 우선순위 | qlog(0) > qcamera(1) > 나머지(1,000). 종량제 통신망에서는 사용자가 앱에서 본 주행만 저화질 영상 업로드. qlog 25MB, qcamera 5MB 상한 | 🔍 `system/loggerd/uploader.py` |
| 원본 당겨오기 | 클라우드가 특정 세그먼트를 지목하면 그때 전체 로그와 풀 화질 영상을 **당겨옴**(`uploadFilesToUrls`, 셀룰러 허용 플래그, 31일 지나면 만료, 최대 30회 재시도) | 🔍 `system/athena/athenad.py` |
| 보존·삭제 | 운전자가 버튼을 누르면(북마크) 세그먼트에 보존 속성이 붙음. 삭제기는 여유 공간이 5GiB 또는 10% 미만일 때만 가장 오래된 것부터 지우되 최근 북마크 5개와 그 앞 2개 세그먼트는 남김 | 🔍 `system/loggerd/deleter.py` |

  - 정리: **"요약을 먼저 올리고, 흥미로운 구간은 서버가 지목해 원본을 당긴다"** 는 두 단계 구조임.
  - 규모 비교: openpilot 설정으로 계산하면 카메라 3대 × 5Mbps ≈ 시간당 약 6.75GB(압축 후)로, Intel의 비압축 추정과 두 자릿수 배 차이가 남.
  - 온보드 설계의 핵심은 "무엇을 올릴지"보다 "무엇을 고화질로 남길지"임.
- **트리거 세 종류**
...
| 모델 점수 | 온보드 소형 분류기가 "가치 있는 장면" 점수를 매기거나, **섀도 모드**의 새 모델과 현행 모델의 출력이 다를 때 | Tesla는 2021년 기준 221개 트리거를 플릿에서 돌렸고, 걸리면 약 10초 클립을 올렸음 📰 ([Karpathy CVPR'21 요약](https://dynamicallytyped.com/stories/2021/karpathy-autopilot-cvpr/)); NVIDIA는 모델 앙상블의 불일치로 고른 데이터가 수동 선별보다 야간 보행자 검출을 3배, 자전거를 4.4배 개선했다고 보고 📰 ([NVIDIA 블로그](https://medium.com/nvidia-ai/scalable-active-learning-for-autonomous-driving-a-practical-implementation-and-a-b-test-4d315ed04b5f)) |
| 사람 | 운전자 북마크, 테스트 드라이버 태깅 | openpilot `userBookmark` 🔍 |

- **사례 적용**
  - 운전자가 브레이크를 밟은 순간 개입 플래그가 찍힘. 규제용 기록(DSSAD)과 개발용 로거 양쪽에 남음.
  - 동시에 "급감속 + 보행자 클래스의 신뢰도가 직전 프레임 대비 급등 + 야간 + 와이퍼 켜짐"이라는 복합 트리거가 걸림. 또는 섀도 모드로 실려 있던 후보 모델이 작업자를 더 일찍 봤다면 두 모델의 출력 차이가 트리거가 됨.
  - 요약 로그와 저화질 영상이 먼저 올라가고, 서버가 이 세그먼트를 지목해 풀 화질 영상과 전체 로그를 당겨옴.

> **시사점.** 온보드 선별은 가장 싼 비용 절감 수단임(올리지 않은 데이터는 저장·반출·라벨링 비용이 0, 5.3절). 규제용 기록(DSSAD)과 개발용 트리거가 같은 이벤트를 찍으므로 한 파이프라인으로 설계하는 것이 자연스러움(5.6절).

### 3.2 2단계 — 검색·라벨링 (클라우드)

- **원칙: 하나를 찾았으면 비슷한 것을 모음**
  - 클립 하나로는 모델을 못 고침. 플릿 전체에서 "비 오는 밤 공사 구간의 서 있는 사람"과 비슷한 장면을 수백~수천 개 모아야 함.
  - 수단은 **임베딩 검색**임. 영상을 숫자 벡터로 바꿔 두면 글("worker standing next to traffic cone at night in rain")이나 다른 영상으로 비슷한 것을 찾을 수 있음.
  - 핵심 가치: 트리거에 걸리지 않았던 "개입은 없었지만 아슬아슬했던" 장면까지 회수 가능함.
- **도구**

| 도구 | 내용 | 근거 |
|---|---|---|
| NVIDIA Cosmos Dataset Search | 오픈 블루프린트. 영상·텍스트를 같은 공간에 놓는 임베딩 모델(Cosmos-Embed1) + GPU 가속 벡터 DB(Milvus + cuVS) + Ray 기반 배치 수집 파이프라인. 임베딩 모델은 약 800만 영상으로 학습 | 🔍 ([README](https://raw.githubusercontent.com/NVIDIA-Omniverse-blueprints/cosmos-dataset-search/main/README.md)); 800만 영상은 📰 |
| Voxel51 FiftyOne | 오픈소스 데이터셋 도구. README: "데이터 문제·라벨 오류·엣지 케이스를 빠르게 찾고 고친다" | 🔍 ([README](https://raw.githubusercontent.com/voxel51/fiftyone/develop/README.md)) |
| NVIDIA Cosmos Curator | 분할 → 필터 → 캡션 → 임베딩 → 중복 제거 → 데이터셋 생성 파이프라인. NVIDIA 발표: 2,000만 시간 영상을 Blackwell GPU로 14일(Hopper 40일, CPU로는 3년 이상). 단, README에는 처리량 수치가 없음 | 🔍 ([README](https://raw.githubusercontent.com/nvidia-cosmos/cosmos-curate/main/README.md)); 처리량은 📰 ([NVIDIA 뉴스룸](https://nvidianews.nvidia.com/news/nvidia-launches-cosmos-world-foundation-model-platform-to-accelerate-physical-ai-development)) |
| 시각언어모델(VLM) 캡션 | 영상에 글 설명을 붙여 검색 가능하게 만듦. Cosmos Reason 2(2B/8B/32B)는 "영상 캡션, 시간 구간 찾기, 물리 추론" 수행. Uber가 자율주행 학습 데이터 캡션에 검토 중이라고 AWS 블로그가 전함 | 🔍 ([README](https://raw.githubusercontent.com/nvidia-cosmos/cosmos-reason2/main/README.md)); Uber 건은 📰 |
...
| 주체·도구 | 내용 | 근거 |
|---|---|---|
| Tesla(AI Day 2022) | 여러 주행을 합쳐 4D(3D + 시간)로 재구성한 뒤 차선·객체 라벨을 자동 생성. 1만 주행 기준 "수작업 500만 시간 → 클러스터 12시간" 주장(자동 라벨링용 GPU 4,000장) | 📰⚠️ ([요약](https://www.nocode.ai/recap-tesla-ai-day-2022/)) |
| Waymo 3D 자동 라벨링(CVPR 2021) | 포인트클라우드 시퀀스 전체를 써서 만든 3D 박스가 사람 라벨과 대등 | 📰 ([arXiv 2103.05073](https://arxiv.org/abs/2103.05073)) |
| Grounding DINO(오픈) | 글 프롬프트("construction worker . traffic cone")로 박스 생성. COCO 제로샷 52.5 AP | 🔍 ([README](https://raw.githubusercontent.com/IDEA-Research/GroundingDINO/main/README.md)) |
| SAM 2(오픈) | 점·박스 프롬프트로 영상 전체의 마스크와 추적 생성. tiny 38.9M ~ large 224.4M 파라미터, A100에서 91.2~39.5 FPS | 🔍 ([README](https://raw.githubusercontent.com/facebookresearch/sam2/main/README.md)) |
| 라벨 형식 | ASAM OpenLABEL(2021-11): JSON으로 2D/3D 박스·폴리곤과 "객체·행동·사건·맥락" 태그를 통일한 표준 | 📰 ([ASAM](https://www.asam.net/news-media/news/detail/news/asam-releases-asam-openlabel-v100/)) |
...
- **사람의 역할 = 검수로 이동**
  ...
  - 외주 라벨 단가: 2D 박스 개당 $0.03~1.00, 세그멘테이션 마스크 $0.05~3.00 범위. Scale AI 같은 회사는 공개 단가표가 없음 📰⚠️ ([업계 가이드](https://www.basic.ai/blog-post/how-much-do-data-annotation-services-cost-complete-guide-2025)).
- **사례 적용**
  - 클립을 임베딩해 플릿 전체에서 유사 장면을 검색하고, VLM으로 "roadwork, worker, night, rain" 태그를 붙여 걸러 냄.
  - 자동 라벨러가 미래 프레임과 여러 주행을 합쳐 작업자의 3D 박스를 첫 프레임부터 만듦.
  - 희귀 클래스이므로 사람이 전수 검수한 뒤 OpenLABEL로 저장함.

> **시사점.** 라벨 비용이 사람 수에서 GPU 시간으로 이동하되, 검수 인력은 남음. 희귀 클래스일수록 검수 비율을 높여야 정확도 "착시"를 피할 수 있음.

### 3.3 3단계 — 학습·합성 데이터 (클라우드)

- **전제: 모은 것만으로는 부족함**
  - 첫 번째 방법: 수백 개 클립을 오버샘플링해 학습 비중을 높임. Tesla FSD v14.3 릴리스 노트는 "강화학습 단계를 업그레이드해 어려운 예에 집중"했다고 적음 📰⚠️ ([릴리스 노트 인용](https://www.notateslaapp.com/software-updates/version/2026.2.9.6/release-notes)).
...
|---|---|---|---|
| **재생(replay)** | 기록된 로그를 그대로 틀고 새 모델을 돌림. 다른 차들은 기록대로 움직임(비반응) | 가장 싸고 정확하지만, 내 차가 다르게 움직여도 남들이 반응하지 않아 인과가 끊김. "기록된 틈을 기다리는" 편법을 배울 위험 📰 ([분석](https://arxiv.org/pdf/2510.14677)) | nuPlan, NAVSIM 🔍 |
| **재구성(reconstruction)** | 실제 로그를 3D 장면으로 복원해 다른 궤적·시점에서 다시 렌더링 | 실제 장면 기반이라 현실감이 높고 "그때 왼쪽으로 꺾었다면"을 볼 수 있음. 기록 범위 밖 시점은 품질 저하 | 3D 가우시안 스플래팅(1080p 30fps 이상, 학습에 24GB VRAM) 🔍 ([README](https://raw.githubusercontent.com/graphdeco-inria/gaussian-splatting/main/README.md)); NVIDIA 3DGRUT(어안·롤링셔터·반사까지 처리, USD/NuRec 내보내기) 🔍; **InstantNuRec**은 10~20초 다중 카메라 장면을 약 1.5초에 재구성하고 Waymo Open Dataset에서 기존 최고 대비 +2.01dB 🔍 ([README](https://raw.githubusercontent.com/NVIDIA/instant-nurec/main/README.md)); Waymo Block-NeRF는 사진 280만 장으로 샌프란시스코 한 구역을 35개 신경 모델로 복원 📰 |
| **생성(generation)** | 조건(지도·라이다·세그멘테이션)을 주고 없던 영상을 만듦 | 밤·비·작업자 조합처럼 존재하지 않는 교차 조건을 만들 수 있는 유일한 길. 현실과의 격차 검증이 필수 | NVIDIA Cosmos Transfer1은 세그멘테이션·깊이·엣지, 자율주행용 라이다·HD맵 조건으로 영상을 만들고, 영상 1개를 다시점으로 늘리며, 2025-08 증류 모델은 36스텝을 1스텝으로 줄임 🔍 ([README](https://raw.githubusercontent.com/nvidia-cosmos/cosmos-transfer1/main/README.md)); Wayve GAIA-3(150억 파라미터, 데이터 10배)는 안전 관련 희귀 장면을 만들어 평가하며 "합성 테스트 기각률 5배 감소"를 주장 📰⚠️; Waymo World Model 📰 |
...
| 사례 | 내용 | 근거 |
|---|---|---|
| NVIDIA AlpaGym | "시뮬레이터 안에서 정책을 폐루프로 돌리고, 주행을 채점하고, 그것으로 학습한다"는 오픈 프레임워크. AlpaSim(시뮬레이터) + Cosmos-RL(학습기) 조합. 현재 Alpamayo 1.5(10B) 정책을 GPU 2장 추론으로 지원하며 "초기 개발 단계"라고 밝힘 | 🔍 ([README](https://raw.githubusercontent.com/NVlabs/alpagym/main/README.md)) |
| comma.ai openpilot | 2022-11 0.9.0 "재투영 시뮬레이터로 학습" → 2025-08 0.10.0 "월드모델 기반 종단간 계획이 종방향 MPC 대체" → 2026-03 0.11.0 "**학습된 시뮬레이터로 전량 학습한** 새 주행 모델" 배포. 블로그: 이 월드모델은 2B 파라미터, 주행 영상 250만 분으로 학습. 합성 환경에서 학습한 정책을 실제 사용자에게 배포한 드문 공개 사례 | 🔍 ([RELEASES.md](https://raw.githubusercontent.com/commaai/openpilot/master/RELEASES.md)); 2B·250만 분은 📰 |
...
- **사례 적용**
  ...
  - Cosmos Transfer로 같은 지도·라이다 조건에서 강우 강도를 바꿈.
  - 월드모델로 "콘 뒤에서 작업자가 나오는" 존재하지 않는 조합을 만듦.
  - 재구성 장면 안에서 정책을 굴려 "더 일찍 감속"에 보상을 줌.

> **시사점.** 재생 → 재구성 → 생성으로 갈수록 "없던 상황"을 만들 수 있지만 현실과의 격차 검증 부담이 커짐(5.2절). 조건부 변환은 기하를 실제로 유지해 그 부담을 줄이는 절충안임.

### 3.4 4단계 — 검증·배포

- **전제: 개루프 점수는 이 사례를 못 잡음**
  - 개루프(open-loop) 평가 = 기록된 궤적과 예측 궤적의 거리(ADE/FDE)로 채점.
  - 사례의 기록 궤적은 "늦게 본 운전자"의 궤적이므로, 그것과 가까울수록 좋은 점수를 받는다면 늦게 보는 모델이 이김.
  - 따라서 검증은 사다리처럼 올라감.
- **검증 사다리**
...
- **규제가 마지막 단을 관리 프로세스로 만듦**
  - UN R156: 소프트웨어 업데이트 관리체계(SUMS)를 형식승인 조건으로 요구. 요건 = 모든 소프트웨어 버전의 고유 식별(RxSWIN), 무결성 검증, 대상 차량 구성과의 호환성 확인, 주행 중 안전한 실행, 사용자 고지, 기록. EU는 2024년 7월부터 모든 신차 적용 📰 ([UL 해설](https://www.ul.com/sis/insights/software-update-management-systems-according-unece-r156)).
...
- **사례 적용**
  ...
  - 배포된 새 모델이 다음 실패를 만들면 다시 1단계임.

> **시사점.** 검증은 한 지표가 아니라 사다리임. 각 단은 이전 단이 못 보던 것을 보며, 마지막 단(OTA)은 규제 프로세스이므로 개발 파이프라인과 인증 산출물을 함께 설계해야 함.
...
- **미공개 항목**: "트리거에서 배포까지 몇 주"라는 명시적 수치는 어느 회사도 공개하지 않음. 확인되는 것은 "수주 단위 OTA"라는 정성 표현뿐임 ⚠️.

> **시사점.** 마디별 처리량은 공개돼 있으나 한 바퀴의 소요 시간은 어디에도 없음. 자사 플라이휠을 설계할 때는 이 "한 바퀴 시간"을 핵심 지표로 직접 재야 함.

---
## 4. 기업별 데이터 플라이휠 전략과 최근 산업 동향

> **개요.** 회사마다 플라이휠을 어떻게 다르게 돌리는지를 데이터 원천 기준 다섯 유형으로 나눠 비교하고, 공통으로 향하는 흐름과 2025~2026 동향을 정리함.
>
> **핵심 결론.** ① 데이터가 어디서 오느냐로 다섯 유형이 갈림. 양산차 플릿(Tesla·comma), 로보택시(Waymo·Baidu), 공급자(Mobileye·Wayve·Momenta), OEM 자체(현대차·42dot, XPeng), 툴체인(NVIDIA·Applied Intuition·TIER IV). ② 공통 흐름은 "월드모델이 시뮬레이션의 중심으로, 강화학습이 양산으로, 라벨링은 자동으로, 데이터 주권이 아키텍처를 가르고, 툴체인은 열린다"는 다섯 가지임. ③ 이 장의 수치는 대부분 기업 발표이므로 "공개 주장"으로 읽어야 함.
...
- **Tesla** — 가장 큰 플릿을 가진 회사

| 항목 | 내용 | 근거 |
|---|---|---|
| 데이터 규모 | FSD(Supervised) 누적 주행 2026년 5월 100억 마일, 8월 140억 마일 돌파 발표(하루 약 3,500만 마일 추정) | 📰⚠️ ([Electrek](https://electrek.co/2026/05/03/tesla-fsd-10-billion-miles-no-magical-milestone-autonomy/)) |
| 수집 방식 | FSD를 켜지 않은 차에서도 백그라운드에서 판단을 내려 실제 운전과 비교하는 섀도 모드 가동 | 📰 |
| 학습 인프라 | Cortex 2026년 1분기 "H100 상당 10만 장 이상", Cortex 2 추가 가동. 2026년 설비투자 가이던스 250억 달러 이상(2025년 85억의 약 3배) | 📰⚠️ ([TechCrunch](https://techcrunch.com/2026/04/22/tesla-just-increased-its-capex-to-25b-heres-where-the-money-is-going/)) |
| 배포 주기 | 대형 버전 약 10개월(v13 2024-12 → v14 2025-10), 포인트 릴리스는 1주 간격까지 단축 | 📰 |
| 데이터 주권 | 중국 데이터는 국외 반출이 막혀 상하이 데이터센터에서 따로 학습 | 📰 |
| 공개도 | 데이터·시뮬레이터·모델 모두 비공개 | — |

- **comma.ai openpilot** — 플릿형의 열린 표본

| 항목 | 내용 | 근거 |
|---|---|---|
| 데이터 규모 | 사용자 2만 명 이상, 누적 3억 마일 이상(56%가 openpilot 주행) | 📰 ([comma.ai](https://www.comma.ai/openpilot)) |
| 진화 이력(릴리스 노트) | 2025-08 0.10.0 "종방향 MPC를 월드모델 종단간 계획으로 대체" → 2025-09 0.10.1 "4배 많은 세그먼트로 학습" → 2026-03 0.11.0 "학습된 시뮬레이터로 전량 학습" → 2026-08 0.11.2 "880M 파라미터 대형 모델" | 🔍 ([RELEASES.md](https://raw.githubusercontent.com/commaai/openpilot/master/RELEASES.md)) |
| 배포 주기 | 릴리스 간격 약 2개월 | 🔍 |
| 공개도 | 코드 MIT | 🔍 |

> **시사점.** 플릿형의 강점은 데이터 양과 섀도 모드임. 같은 유형 안에서도 Tesla(폐쇄)와 comma(오픈소스)는 공개도가 정반대이며, comma의 릴리스 노트가 플릿형 플라이휠의 진화를 코드 수준에서 확인할 수 있는 유일한 창임.
...
| 항목 | 내용 | 근거 |
|---|---|---|
| 데이터 규모 | 2026년 3월 말 기준 무인 누적 2억 2,060만 마일, 주 400만 마일 이상 | 📰 ([Waymo](https://waymo.com/safety/impact/)) |
| 안전 실적 | 안전 허브: 사람 대비 중상·사망 충돌 94% 감소, 에어백 전개 82% 감소, 부상 충돌 82% 감소(5개 도시). IIHS 독립 연구: 부상 충돌 81% 감소 | 📰 |
| 시뮬레이션 | "수천만 실주행 마일 + 수십억 시뮬 마일". 2026-02 Genie 3 기반 Waymo World Model로 카메라와 라이다를 동시에 생성해 플릿이 본 적 없는 상황을 만듦 | 📰 ([Waymo 블로그](https://waymo.com/blog/2026/02/the-waymo-world-model-a-new-frontier-for-autonomous-driving-simulation/)) |
| 공개 자산 | 롱테일 데이터셋 WOD-E2E(4,021 세그먼트, 발생 빈도 0.03% 미만) 공개; 안전 케이스 접근 백서(UL 4600 계열) 발간 | 📰 |
| 2026년 목표 | 10개 이상 도시 추가, 런던 진출, 연말 주 100만 회 탑승 | 📰⚠️ |

- **Baidu Apollo Go**

| 항목 | 내용 | 근거 |
|---|---|---|
| 규모 | 2026년 2분기 완전 무인 탑승 약 100만 회, 6월 누적 2,300만 회 | 📰 ([Baidu 6-K](https://www.sec.gov/Archives/edgar/data/1329099/000119312526110843/d34060dex991.pdf)) |
| 안전 실적(공개 주장) | 무인 차량의 에어백 전개는 평균 1,440만 km당 1회 | 📰 |
| 모델·확장 | 6세대 RT6는 자체 파운데이션 모델 ADFM 탑재. 2026년부터 Lyft와 독일·영국에 투입 | 📰 |

- **Zoox**: 2026년 7월 30일 NHTSA에서 핸들 없는 로보택시의 첫 상업 면제(연 2,500대) 획득 📰 ([TechCrunch](https://techcrunch.com/2026/07/30/zoox-clears-final-federal-hurdle-to-launch-paid-robotaxi-service/)).
...
> **시사점.** 로보택시형은 플릿이 작은 대신 센서가 많고 시뮬레이션 비중이 크며, 플릿 통계를 안전 케이스로 직접 연결함(안전 허브·백서). 규제 면제(Zoox)가 배포 관문의 새 선례임.
...
| 항목 | 내용 | 근거 |
|---|---|---|
| 데이터 원천 | REM(Road Experience Management) 크라우드소싱 지도. 18개 브랜드·50개 차종·800만 대 이상의 양산 ADAS 차량이 km당 약 10KB의 익명 데이터를 전송 | 📰 ([Mobileye REM](https://www.mobileye.com/technology/rem/)) |
| 산출물 | 10cm 정확도의 "Roadbook" 지도를 만들어 다시 내려보냄 | 📰 |
| 검증 | "True Redundancy"(카메라 계열과 레이더·라이다 계열을 독립 운영)로 필요한 검증 데이터 양을 줄인다고 주장 | 📰 |
| 규모 | 2026년 EyeQ 칩 출하 약 3,800만 개 가이던스, Surround ADAS 누적 수주 1,900만 대 이상 | 📰 |

- **Wayve**

| 항목 | 내용 | 근거 |
|---|---|---|
| 핵심 자산 | 월드모델 GAIA 시리즈. GAIA-3(2025-12): 150억 파라미터, 이전 세대의 10배 데이터, 검증용으로 외부 제공. GAIA-4(2026-08): AI Driver를 루프 안에 넣어 기록된 장면에서 출발하는 반사실 시나리오 생성 | 📰 ([Wayve](https://wayve.ai/thinking/gaia-4/)) |
| 일반화(공개 주장) | 단일 모델로 유럽·북미·일본 506개 도시 주행, 그중 219개(43%)는 현지 데이터 없이 주행 | 📰⚠️ |
| 상용화 | Nissan LEAF 기반 로보택시로 2026년 말 도쿄 Uber 파일럿 계획 | 📰 |

- **Momenta**

| 항목 | 내용 | 근거 |
|---|---|---|
| 규모 | 양산 탑재 차량 100만 대 돌파(2025년 30만 → 80만 → 상장 직전 100만) | 📰 ([Momenta](https://momenta.cn/en/article/568.html)) |
| 모델(공개 주장) | R6 "플라이휠 대형 모델": 고가치 클립 7,000만 개와 실주행 30억 km로 학습한 강화학습 기반 종단간 모델. R7(2026-04): 사전학습·시뮬레이션·강화학습 3층 구조, 누적 120억 km와 "골든 데이터" 1억 건 | 📰 |
| 고객 | 2026년 2월 기준 글로벌 OEM 24곳(BMW 중국·아우디·도요타·BYD·포드 등) | 📰 |
| 자금 | 2026년 7월 홍콩 상장, 7억 5,000만 달러 조달 | 📰 |
...
> **시사점.** 공급자형은 여러 OEM의 차에서 데이터를 받으므로 지역·차종 다양성이 강점이고, 그 데이터로 만든 지도(REM)·월드모델(GAIA)·플라이휠 모델(R6/R7)이 곧 상품임.
...
| 항목 | 내용 | 근거 |
|---|---|---|
| 발표 | 2026년 9월 13일 "AI 기반 데이터 플라이휠 본격 가동". 수집 → 학습 → 검증 → 배포의 순환을 자율주행 전략의 핵심 엔진으로 설정 | 📰 ([현대차그룹](https://www.hyundaimotorgroup.com/en/news/hyundai-motor-group-accelerates-autonomous-driving-innovation-with-ai-powered-data-flywheel)) |
| 수집 | 42dot이 개발한 종단간 시스템 Atria AI를 2026년 말까지 국내 차량에 실어 엣지 케이스 데이터 수집. 국토부와 협력해 전남·광주에 Atria AI 탑재 "SDV Pace Car" 투입 | 📰 |
| 이중 트랙 | NVIDIA 검증 플랫폼 기반 L2+/L2++ 양산 2028년, 자체 Atria AI 기반 L2++ 양산 2029년 하반기 목표 | 📰 |
| NVIDIA 협력 | 2026년 3월 GTC에서 DRIVE AGX Thor·Hyperion 기반 양산과 Motional을 통한 L4 발표 | 📰 |

- **현대모비스**: 2026년 4월 실차 주행·주차 센서 데이터를 데이터 관리 솔루션과 시뮬레이터에 통합한 검증 체계 발표. 시뮬레이터 60대를 연결하면 "1만 시간 상당 검증을 1주일에" 끝내는 것이 목표이며, 야간·우천·돌발 상황 재현이 강점이라고 밝힘(계획치) 📰⚠️ ([PR Newswire](https://www.prnewswire.com/news-releases/hyundai-mobis-develops-data-driven-validation-system-to-dramatically-cut-testing-time-for-sdvs-302745490.html)).
- **다른 OEM**

| OEM | 내용 | 근거 |
|---|---|---|
| Toyota·Woven | Arene: 동의 기반 주행 데이터 수집·분석(Arene Data)과 가상 검증(Arene Tools)을 2025년 RAV4부터 적용 | 📰 |
| VW | Bosch와의 자율주행 얼라이언스에 약 15억 유로를 쓴 뒤 2026년 7월 조기 종료 확인 | 📰 ([electrive](https://www.electrive.com/2026/07/02/vw-confirms-end-of-automated-driving-alliance-with-bosch/)) |
| Mercedes | 중국 CLA에 Momenta, 미국 CLA에 NVIDIA DRIVE AV를 쓰는 지역 분할 | 📰 |
| XPeng | 자체형의 극단. 롱테일 클립 약 1억 개, 학습비 20억 위안 이상, 제어 가능한 생성 월드모델 X-World를 폐루프 시뮬·온라인 강화학습·데이터 합성에 통합 | 📰⚠️ |
| Li Auto | MindVLA-o1을 데이터 엔진(MindData)·월드모델(MindSim)·강화학습 인프라로 구성 | 📰 |

> **시사점.** OEM 자체형은 데이터 주권과 수직 통합이 동기이나, 플릿·인프라를 스스로 갖춰야 하므로 비용 부담이 큼(VW·Bosch 종료가 실패 비용의 예). 현대차그룹은 NVIDIA 플랫폼과 자체 Atria AI의 이중 트랙으로 위험을 나눔.
...
| 항목 | 내용 | 근거 |
|---|---|---|
| Physical AI Data Factory Blueprint(2026-03) | Cosmos Curator(대규모 처리·주석) → Cosmos Transfer(날씨·조명·환경 변형으로 희소 시나리오 증폭) → Cosmos Reason/Evaluator(물리 정확성 자동 점수) → OSMO 오케스트레이션(코딩 에이전트 연동). Uber·Skild AI 등 채택 | 📰 ([NVIDIA 뉴스룸](https://nvidianews.nvidia.com/news/nvidia-announces-open-physical-ai-data-factory-blueprint-to-accelerate-robotics-vision-ai-agents-and-autonomous-vehicle-development)) |
| 오픈 자산 | Alpamayo(100억 파라미터 추론형 VLA, 인과 추론 자동 라벨링, 1,700시간 이상 공개 주행 데이터), Alpamayo 2 Super(340억), AlpaSim·AlpaGym, Omniverse NuRec | 📰🔍 |
| Uber 협력 | Hyperion 기반 L4 10만 대(2027~)와 Cosmos 기반 "AI 데이터 팩토리"로 Uber 플릿 데이터 큐레이션 | 📰 |

- **기타 툴체인**

| 회사 | 내용 | 근거 |
|---|---|---|
| Applied Intuition | 2025년 6월 기업가치 150억 달러에 6억 달러 조달. 상위 20개 OEM 중 18개가 고객, 시뮬·테스트가 매출의 약 1/3이라는 추정 | 📰⚠️ ([Sacra](https://sacra.com/c/applied-intuition/)) |
| Scale AI | 2025년 6월 Meta의 143억 달러 투자 뒤 고객 이탈과 감원. 자율주행 라벨링 비중은 2022년 이후 줄어 LLM 데이터로 이동 | 📰 |
| Voxel51 | 페타바이트급 플릿 로그에서 임베딩·자연어 검색으로 엣지 케이스를 캐고 NuRec과 연동 | 📰 |
| Foretellix | 2025년 10월 NVIDIA DRIVE AV에 시나리오 도구 Foretify 통합 | 📰 |
| TIER IV Co-MLOps | 2024년 출범한 다사 데이터 공유 플랫폼. 2026년 8월 Astemo와 양산급 종단간 모델 공동 개발 MoU. "사람은 요구사항만 정하고 AI 에이전트가 라벨링·정제·학습·평가를 자율 수행하는" 에이전틱 개발 주장 | 📰⚠️ ([TIER IV](https://tier4.co.jp/en/updates/20260805-tieriv-develops-platform-with-astemo)) |
| 표준·데이터 스페이스 | ASAM이 OpenSCENARIO DSL 2.2(2026-03)·OpenLABEL·OpenODD 유지. Catena-X는 2026년 11월 중국–유럽 간 첫 국경 간 자동차 데이터 생태계 운영 발표 | 📰 |

> **시사점.** 툴체인형은 플라이휠의 마디(큐레이션·합성·시뮬·라벨링)를 부품으로 팔며, NVIDIA는 오픈소스로 표준화를 노림. 라벨링 시장(Scale AI)의 축소는 자동 라벨링 전환의 뒷면임.
...
1. **월드모델이 시뮬레이션의 중심이 됨.** Waymo World Model(2026-02), Wayve GAIA-4(2026-08), XPeng X-World, Momenta R7, Huawei의 클라우드 월드모델, comma의 학습된 시뮬레이터. 2026년 한 해에 거의 모든 진영이 "생성 모델로 시험한다"를 선언함 📰.
2. **강화학습·폐루프 학습이 양산에 들어옴.** Momenta R6("강화학습 기반 종단간 양산 최초" 주장), Tesla v14.3 릴리스 노트의 강화학습 단계, NVIDIA AlpaGym, XPeng 온라인 강화학습 📰🔍.
3. **라벨링이 자동으로 이동함.** NVIDIA 인과 추론 자동 라벨링, TIER IV Co-MLOps 자동 라벨링(2026), Bosch의 신경망 자동 라벨링은 3D 박스 비용 최대 70% 절감을 주장 📰⚠️. Scale AI의 자율주행 라벨링 축소가 같은 흐름의 뒷면임.
4. **데이터 주권이 아키텍처를 가름.** Mercedes CLA(중국 Momenta / 미국 NVIDIA), BMW iX3(중국 Momenta / 글로벌 Qualcomm), Tesla 상하이 데이터센터. 어느 나라의 데이터로 어느 나라의 모델을 만드느냐가 공급 구조를 정함 📰.
5. **툴체인은 열리고 데이터 공유는 실험 중임.** NVIDIA Alpamayo·AlpaSim 오픈소스, Waymo WOD-E2E, TIER IV Co-MLOps, Catena-X 중–유럽 확장. 반면 Tesla·Baidu는 폐쇄를 유지함 📰.
...
> **개요.** 플라이휠이 막히는 네 곳(데이터 발견·검증·비용·규제)을 분석하고, 잘 도는 플라이휠이 사업에서 무엇으로 바뀌는지(개발 속도·원가·시장 확장·안전성 입증·장기 검증 자산)와 다음 5년의 방향을 정리함.
>
> **핵심 결론.** ① 병목은 네 곳임. 무엇을 찾아야 할지 모르는 데이터 발견, 시뮬레이션을 얼마나 믿을 수 있는지의 검증, 저장·통신·GPU·라벨링 비용, 개인정보·국외 반출·업데이트 규제. ② 잘 도는 플라이휠은 개발 속도(주 단위 OTA), 원가(자동 라벨링), 시장 확장(현지 데이터 없이 진입), 안전성 입증(안전 허브·안전 케이스), 장기 검증 자산(회귀 세트·시나리오 라이브러리)으로 바뀜. ③ 방향은 월드모델 기반 검증, 학습된 시뮬레이터, 플릿 간 데이터 공유, 에이전트형 개발, "증거로서의 로깅"임.
...
- **문제 정의**: 가장 어려운 데이터는 "아직 본 적 없어서 찾을 줄도 모르는" 데이터임.
  - ISO 21448(SOTIF)은 이를 "모르는 불안전 시나리오(영역 3)"라 부르고, 이 영역을 합리적 노력으로 최대한 줄이는 것을 표준의 핵심으로 둠 📰 ([TÜV SÜD 백서](https://www.tuvsud.com/-/jssmedia/global/pdf-files/whitepaper-report-e-books/tuvsud-sotif.pdf)).
  - 희소성의 수치: Waymo는 수억 마일 플릿에서 발생 빈도 0.03% 미만 상황을 골랐는데 12시간 분량이었음 📰.
- **기술적 대응(두 방향)**
  - 임베딩·자연어 검색으로 "비슷하지만 트리거는 안 걸린" 장면을 회수함(3.2절).
  - 플릿이 못 본 상황은 생성함. Waymo World Model의 목적에 "플릿이 직접 관찰한 적 없는 상황 탐색"이 명시됨 📰.
- **한계와 최신 연구**
  - 불확실성 기반 능동학습은 초기 라벨 집합에 아예 없는 클래스("unknown unknown")를 잡지 못함 📰 ([해설](https://lacuna.tiptreesystems.com/work/active-data-discovery-mining-unknown-data-using-submodular-information-measures/wrk_c7958570151368a9e16a1f703eafc370)).
  - 2026년 연구는 예측 월드모델의 "놀람" 신호로 희소 사건을 고르는 접근을 제안함 📰 ([arXiv 2608.29772](https://arxiv.org/abs/2608.29772)).

> **시사점.** 검색은 "알지만 못 찾은" 데이터를, 생성은 "본 적 없는" 데이터를 겨냥함. 둘 다 "무엇을 모르는지 모르는" 영역은 못 잡으므로 월드모델의 놀람 신호 같은 발견 기법이 다음 과제임.

### 5.2 병목 ② 검증 — 시뮬레이션을 얼마나 믿을 수 있나

- **개루프 점수의 한계**: nuScenes 개루프 평가에서 카메라·라이다 없이 과거 궤적만 외삽해도 최신 모델을 이기는 사례가 보고됨 📰. 정적 데이터셋 점수가 도로 안전으로 이어지지 않는다는 뜻임.
- **중간 해법과 그 한계**

| 접근 | 내용 | 근거 |
|---|---|---|
| NAVSIM v2 의사 시뮬레이션 | 3D 가우시안 스플래팅으로 후속 관측을 합성해 폐루프 시뮬과 상관 0.89를 얻으면서 환경 상호작용을 6배 줄임 | 📰🔍 ([NAVSIM](https://github.com/autonomousvision/navsim/blob/main/README.md)) |
| Bench2Drive-Robust(2026-05) | 프레임 드롭·GPS 잡음·추론 지연 같은 "배포 교란"이 폐루프 성능을 크게 떨어뜨리는데 기존 이미지 손상 평가로는 잡히지 않음 | 📰 ([arXiv 2605.18059](https://arxiv.org/abs/2605.18059)) |
| 자율주행 테스트 서베이(2025-12) | 미해결 과제로 "코너 케이스 다양성, 시뮬–실제 격차, 체계적 기준 부재, 파운데이션 모델 테스트 비용"을 지목 | 📰 ([arXiv 2512.11887](https://arxiv.org/abs/2512.11887)) |
...
- **비용 구조가 말해 주는 것**
  - **차에서 고르는 것이 가장 싼 절감임.** 올리지 않은 데이터는 저장·반출·라벨링 비용이 모두 0임.
  - **라벨링은 사람 수에서 GPU 시간으로 이동 중임.** 자동 라벨링의 70% 절감 주장이 맞다면, 남는 비용은 검수 인력과 GPU임.

> **시사점.** 비용 최적화의 순서는 온보드 선별 → 자동 라벨링 → 반출 최소화(현지 학습)임. 이는 3.1절의 두 단계 업로드와 5.4절의 데이터 주권 대응과 같은 방향임.
...
> **시사점.** 규제는 플라이휠의 세 마디를 직접 건드림. 수집(개인정보·주권), 배포(R156 SUMS), 기록(R157 DSSAD·EDR). 규제를 사후 대응이 아니라 아키텍처 조건으로 넣어야 함.

### 5.5 플라이휠이 사업 경쟁력으로 바뀌는 다섯 경로

| 경쟁력 | 플라이휠이 어떻게 만드나 | 근거 사례 |
|---|---|---|
| **개발 속도** | 트리거 → 배포 한 바퀴가 짧을수록 문제를 빨리 고침 | Tesla 대형 버전 연 1회 + 포인트 릴리스 주 단위; comma 약 2개월; Waymo 2026년 10개 이상 도시 확장(2024년 2개, 2025년 2개 대비) 📰⚠️ |
| **원가** | 자동 라벨링과 온보드 선별이 사람·저장·반출 비용을 줄임 | Bosch 70% 절감 주장 📰⚠️. 주의: NVIDIA의 "98.6% 추론 비용 절감"은 사내 인사 챗봇의 단순 도구 호출을 70B → 1B로 증류한 사례이며 자율주행에 그대로 적용할 수 없음 🔍 |
| **시장 확장** | 새 지역 데이터를 빠르게 흡수해 현지화 비용을 줄임 | Wayve 506개 도시·43% 현지 데이터 없이·HD 지도 불사용 📰⚠️; Momenta 24개 OEM·100만 대 📰; Mobileye Surround ADAS 1,900만 대 수주 📰 |
| **안전성 입증** | 플릿 데이터가 곧 안전 통계이고, 시뮬·회귀 세트가 안전 케이스의 증거가 됨 | Waymo 안전 허브(2.2억 마일, 중상·사망 94% 감소)와 동료 심사 논문 📰; Baidu 에어백 1,440만 km당 1회 📰; Mercedes L3 95km/h는 TÜV Rheinland 시험과 트랙·공도 동적 시험으로 KBA 승인 📰; UL 4600 안전 케이스 📰 |
| **장기 검증 자산** | 한 번 만든 시나리오·회귀 세트는 다음 모델, 다음 차종에도 쓰임 | Waymo Simulation City·Waymax 📰; ASAM OpenSCENARIO DSL 2.2·OpenLABEL·OpenODD로 이식성 확보 📰; TIER IV scenario_simulator_v2 🔍; Foretellix 시나리오 커버리지 📰 |

- **공통 조건**: 다섯 경쟁력의 공통 조건은 **한 바퀴를 얼마나 자주, 얼마나 싸게, 얼마나 믿을 수 있게 돌리느냐**임. 5.1~5.4의 병목이 바로 그 세 변수를 막는 곳임.

> **시사점.** 경쟁력 지표를 "자주(주기)·싸게(바퀴당 비용)·믿을 수 있게(검증 신뢰도)" 세 변수로 정의하면, 병목 해소 우선순위가 그대로 투자 우선순위가 됨.

### 5.6 향후 진화 방향

1. **월드모델 기반 검증이 표준이 됨.** Wayve GAIA-4(AI Driver 인더루프, 반사실 재생), NVIDIA Cosmos Transfer·NuRec, Waymo World Model(라이다 동시 생성), Huawei 클라우드 월드모델, XPeng X-World 📰. 관건은 "월드모델이 만든 장면을 규제가 증거로 받아 주는가"이며, R157 부속서 4의 "도구 검증" 요건이 그 관문임.
2. **학습된 시뮬레이터에서 직접 학습한 모델이 양산차에 실림.** comma 0.11이 첫 공개 사례이고 🔍, NVIDIA AlpaGym·Momenta R7이 같은 방향임 📰.
3. **플릿 간 데이터 공유가 실험 단계를 지남.** TIER IV Co-MLOps(택시 회사·Astemo), Catena-X 중–유럽(2026-11), NVIDIA 1,700시간 오픈 데이터, Waymo WOD-E2E 📰. 데이터 주권 규제가 오히려 "국경 안에서의 공유"를 밀어 올림.
4. **에이전트가 플라이휠을 돌림.** TIER IV는 "사람은 요구사항, AI 에이전트가 라벨링·정제·학습·평가·개선"을 주장하고 📰⚠️, NVIDIA OSMO는 코딩 에이전트와 연동해 재구성·큐레이션을 자동화함 📰. 아직 주장 단계임.
5. **파운데이션 모델이 라벨링을 줄임.** VLM이 차종·치수를 추론해 3D 박스의 초깃값을 만들고, VLM을 "라벨 생성기"로 써서 가벼운 검출기를 학습시킴 📰.
6. **로깅이 규제 증거가 됨.** R157 DSSAD, EU EDR 의무(2024-07), 2022/1426의 L3 이상 데이터 저장 요구, 중국 로그 3년 보관, 한국 접근 기록 의무 📰. 개발용 트리거와 규제용 기록이 같은 이벤트(개입·전환 요구·최소위험기동)를 찍으므로, 둘을 한 파이프라인으로 설계하는 것이 자연스러움.

> **시사점.** 여섯 방향 중 ①②⑤는 기술 성숙도의 문제이고, ③④는 아직 주장 단계이며, ⑥은 이미 의무임. 우선순위는 ⑥(로깅 통합) → ①(월드모델 검증) → ②(학습된 시뮬레이터) 순이 합리적임.

### 5.7 차량 HPC 플랫폼 관점 메모

- **전제**: 플라이휠의 첫 마디와 마지막 마디는 차 안에 있음. 차량용 고성능 컴퓨팅 플랫폼(HPC)이 플라이휠에 제공해야 하는 것을 이 보고서의 근거에서 추리면 다섯 가지임.

| # | 요구 | 근거 |
|---|---|---|
| 1 | **섀도 실행 자원.** 현행 모델과 후보 모델을 동시에 돌리려면 추론 자원의 여유가 필요함 | Tesla 섀도 모드 📰 |
| 2 | **선별·압축·2단계 로깅.** 요약 로그 + 저화질 영상 + 원본 지목 업로드 구조는 저장·통신 예산과 온보드 인코더 자원을 전제로 함 | openpilot 🔍 |
| 3 | **규제 기록의 무결성.** DSSAD·EDR·R156의 버전 식별·이력은 변조 방지 저장과 시간 동기화를 요구함 | 📰 |
| 4 | **HIL 재생 호환.** 원시 센서·버스 데이터를 실제 ECU에 시간 맞춰 주입하는 검증이 되려면 플랫폼의 입력 경로가 재생을 지원해야 함 | dSPACE, 현대모비스 📰 |
| 5 | **OTA 관리 프로세스.** R156의 SUMS·RxSWIN 요건은 플랫폼의 업데이트 경로 설계 조건임 | 📰 |

> **시사점.** 플라이휠은 클라우드만의 일이 아님. 온보드 플랫폼이 섀도 실행·2단계 로깅·규제 기록·HIL 재생·OTA 관리의 다섯 가지를 지원하지 않으면 첫 마디와 마지막 마디가 막힘.
...
| 용어 | 뜻 |
|---|---|
| **데이터 플라이휠** | 서비스를 쓰면서 나온 데이터로 모델을 고치고, 고친 모델이 더 좋은 데이터를 만드는 반복 구조. 자율주행에서는 "데이터 엔진", "폐쇄루프"라고도 부름 |
| **종단간(end-to-end, E2E)** | 카메라 영상이 들어가면 조향·제동이 바로 나오는 하나의 학습 모델. 중간에 사람이 짠 규칙이 없음 |
| **롱테일(long tail)** | 각각은 드물지만 종류가 무한히 많은 예외 상황. 빈도 그래프의 긴 꼬리 |
| **분포 변화(distribution shift)** | 학습할 때 본 상황의 분포와 실제로 만나는 상황의 분포가 다른 것. 도시·날씨·나라가 바뀌면 생김 |
| **파국적 망각(catastrophic forgetting)** | 새 데이터로 다시 학습한 모델이 예전에 잘하던 것을 잊는 현상 |
| **트리거(trigger)** | 차 안에서 "이 순간의 데이터를 저장·업로드하라"고 판단하는 조건. 규칙, 모델 점수, 사람 버튼이 있음 |
...
| **월드모델(world model)** | 지금 장면과 행동을 주면 다음 장면을 예측·생성하는 모델. 시뮬레이터로 쓰임 |
...
| **OTA(Over-the-Air)** | 무선으로 차량 소프트웨어를 갱신하는 것. UN R156이 관리 방식을 규정함 |
...
| 16 | 발표 슬라이드 화면(Tesla·Waymo·Momenta)의 발표 행사·연도 | 수록 저장소에 기재 없음 |

## 부록 C. 출처

- 전체 출처 목록(URL·등급·확인한 사실·접근 실패 목록): [reference/references.md](reference/references.md). 이 개조식 판은 줄글 판과 같은 출처 집합을 씀.
- 그림 출처(23장, 라이선스·원본 크기·축소 여부): [reference/images.md](reference/images.md).
